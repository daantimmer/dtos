...
namespace type_safe
{
    ...
    struct strong_typedef
    {
        ...
        template <class O>
        explicit constexpr strong_typedef(const O&) = delete;
        ...
        template <class O>
        explicit constexpr strong_typedef(O&&) = delete;

        /// \group value_ctor
        explicit constexpr strong_typedef(T&& value) noexcept(std::is_nothrow_move_constructible<T>::value)
    ...
    namespace strong_typedef_op
    {
        ...
        namespace detail
        {
            ...
            using enable_if_convertible =
                typename std::enable_if<!std::is_same<typename std::decay<From>::type, To>::value
...
#define TYPE_SAFE_DETAIL_MAKE_OP(Op, Name, Result)                                                                   \
    ...
    {                                                                                                                \
        return Result(detail::get_underlying<StrongTypedef>(static_cast<const StrongTypedef&>(lhs))                  \
    ...
    /* mixed */                                                                                                      \
    /** \exclude */                                                                                                  \
    template <class StrongTypedef, typename Other, typename = detail::enable_if_convertible<Other&&, StrongTypedef>> \
    ...
    {                                                                                                                \
        return Result(detail::get_underlying<StrongTypedef>(detail::forward<Other>(lhs))                             \
                          Op detail::get_underlying<StrongTypedef>(static_cast<const StrongTypedef&>(rhs)));         \
    }                                                                                                                \
    /** \exclude */                                                                                                  \
    template <class StrongTypedef, typename Other, typename = detail::enable_if_convertible<Other&&, StrongTypedef>> \
...
#define TYPE_SAFE_DETAIL_MAKE_OP_COMPOUND(Op, Name)                                                                  \
    /** \exclude */                                                                                                  \
    template <class StrongTypedef>                                                                                   \
    constexpr StrongTypedef& operator Op(Name<StrongTypedef>& lhs, const Name<StrongTypedef>& rhs)                   \
    {                                                                                                                \
    ...
    template <class StrongTypedef>                                                                                   \
    constexpr StrongTypedef& operator Op(Name<StrongTypedef>& lhs, Name<StrongTypedef>&& rhs)                        \
    {                                                                                                                \
        detail::get_underlying<StrongTypedef>(static_cast<StrongTypedef&>(lhs))                                      \
            Op detail::get_underlying<StrongTypedef>(static_cast<StrongTypedef&&>(rhs));                             \
        return static_cast<StrongTypedef&>(lhs);                                                                     \
    ...
    /** \exclude */                                                                                                  \
    template <class StrongTypedef, typename Other, typename = detail::enable_if_convertible<Other&&, StrongTypedef>> \
    constexpr StrongTypedef& operator Op(Name<StrongTypedef>& lhs, Other&& rhs)                                      \
    {                                                                                                                \
        detail::get_underlying<StrongTypedef>(static_cast<StrongTypedef&>(lhs))                                      \
            Op detail::get_underlying<StrongTypedef>(detail::forward<Other>(rhs));                                   \
        return static_cast<StrongTypedef&>(lhs);                                                                     \
...
#define TYPE_SAFE_DETAIL_MAKE_OP_COMPOUND_MIXED(Op, Name)                                                     \
    ...
    template <class StrongTypedef,                                                                            \
              ...
              typename Other,                                                                                 \
              typename = detail::enable_if_convertible_same<Other&&, OtherArg>>                               \
    constexpr StrongTypedef& operator Op(Name<StrongTypedef, OtherArg>& lhs, Other&& rhs)                     \
    {                                                                                                         \
        get(static_cast<StrongTypedef&>(lhs)) Op detail::forward_or_underlying(detail::forward<Other>(rhs));  \
        return static_cast<StrongTypedef&>(lhs);                                                              \
    }                                                                                                         \
    /** \exclude */                                                                                           \
    template <class StrongTypedef,                                                                            \
...
#define TYPE_SAFE_DETAIL_MAKE_STRONG_TYPEDEF_OP(Name, Op)                                          \
    ...
    TYPE_SAFE_DETAIL_MAKE_OP_COMPOUND_MIXED(Op## =, mixed_##Name##_noncommutative)
        ...
        struct increment
        {
            ...
            /// \exclude
            constexpr StrongTypedef operator++(int)
            {
        ...
        struct input_operator
        {
            ...
            friend std::basic_istream<Char, CharTraits>& operator>>(std::basic_istream<Char, CharTraits>& in,
                                                                    StrongTypedef& val)
            {
                using type = underlying_type<StrongTypedef>;
        ...
        struct output_operator
        {
            /// \exclude
            template <typename Char, class CharTraits>
            friend std::basic_ostream<Char, CharTraits>& operator<<(std::basic_ostream<Char, CharTraits>& out,
