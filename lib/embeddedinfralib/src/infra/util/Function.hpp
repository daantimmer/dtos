...
#define UTIL_FUNCTION_ALIGNMENT uint64_t
...
namespace infra
{
    ...
    namespace detail
    {
        ...
        struct InvokerFunctions<Result(Args...), ExtraSize>
        {
            ...
            typedef typename std::aligned_storage<ExtraSize, std::alignment_of<UTIL_FUNCTION_ALIGNMENT>::value>::type
                StorageType;
            StorageType data;
            ...
            template <class F>
            static const VirtualMethodTable*
                StaticVirtualMethodTable(typename std::enable_if<std::is_trivial<F>::value>::type* = 0);
            template <class F>
            static const VirtualMethodTable*
                StaticVirtualMethodTable(typename std::enable_if<!std::is_trivial<F>::value>::type* = 0);
            template <class F>
    ...
    public:
        ...
        explicit Execute(const Function<void()>& f);
    };
    ...
    public:
        ...
        explicit ExecuteOnDestruction(const Function<void()>& f);
    ...
    namespace detail
    {
        template <std::size_t ExtraSize, class Result, class... Args>
        template <class F>
        Result InvokerFunctions<Result(Args...), ExtraSize>::StaticInvoke(const InvokerFunctionsType& invokerFunctions,
                                                                          Args... args)
        {
        ...
        template <std::size_t ExtraSize, class Result, class... Args>
        template <class F>
        void InvokerFunctions<Result(Args...), ExtraSize>::StaticCopyConstruct(const InvokerFunctionsType& from,
                                                                               InvokerFunctionsType& to)
        {
            new (&to.data) F(reinterpret_cast<const F&>(from.data)); // TICS !OAL#011
            std::memset(reinterpret_cast<char*>(&to.data) + sizeof(F),
                        0,
                        ExtraSize - sizeof(F)); // TICS !COV_CPP_NO_EFFECT_13
        ...
        const typename InvokerFunctions<Result(Args...), ExtraSize>::VirtualMethodTable*
            InvokerFunctions<Result(Args...), ExtraSize>::StaticVirtualMethodTable(
                typename std::enable_if<std::is_trivial<F>::value>::type*)
        {
        ...
        {
            static_assert(sizeof(F) <= ExtraSize,
                          "Not enough static storage availabe for construction of derived type");
            static_assert(std::alignment_of<F>::value <= sizeof(UTIL_FUNCTION_ALIGNMENT),
                          "Alignment of U is larger than alignment of this function");

            new (&invokerFunctions.data) F(std::forward<F>(f)); // TICS !OAL#011
            std::memset(reinterpret_cast<char*>(&invokerFunctions.data) + sizeof(F),
                        0,
                        ExtraSize - sizeof(F)); // TICS !COV_CPP_NO_EFFECT_13
    ...
    template <std::size_t ExtraSize, class Result, class... Args>
    typename Function<Result(Args...), ExtraSize>::ResultType
        Function<Result(Args...), ExtraSize>::operator()(Args... args) const
    {
    ...
    namespace detail
    {
        ...
        struct TupleInvokeHelper
        {
            ...
            static ResultType Invoke(const InvokerFunctions& invokerFunctions,
    ...
    template <std::size_t ExtraSize, class Result, class... Args>
    typename Function<Result(Args...), ExtraSize>::ResultType
        Function<Result(Args...), ExtraSize>::Invoke(const std::tuple<Args...>& args) const
    {
        return detail::TupleInvokeHelper<ResultType, Args...>::Invoke(invokerFunctions,
                                                                      invokerFunctions.virtualMethodTable->invoke,
                                                                      args,
