...
namespace infra
{
    ...
    class Observer: public IntrusiveList<Observer<Descendant, SubjectType_>>::NodeType
    {
    public:
    ...
    public:
        template <class F,
                  std::enable_if_t<std::is_same_v<typename std::invoke_result_t<F, ObserverType_&>, void>, int> = 0>
        ...
        template <class F,
                  std::enable_if_t<!std::is_same_v<typename std::invoke_result_t<F, ObserverType_&>, void>, int> = 0>
        bool NotifyObservers(F callback) const;

        template <class F,
                  std::enable_if_t<!std::is_same_v<typename std::invoke_result_t<F, ObserverType_&>, void>, int> = 0>
        bool NotifyObservers(F callback);
    ...
    template <class Descendant, class SubjectType_>
    typename SingleObserver<Descendant, SubjectType_>::SubjectType&
        SingleObserver<Descendant, SubjectType_>::Subject() const
    {
    ...
    template <class ObserverType>
    template <class F, std::enable_if_t<std::is_same_v<typename std::invoke_result_t<F, ObserverType&>, void>, int>>
    void Subject<ObserverType, void>::NotifyObservers(F callback) const
    {
        for (typename IntrusiveList<Observer<ObserverType, SubjectType>>::const_iterator i = observers.begin();
             i != observers.end();)
        {
    ...
    {
        for (typename IntrusiveList<Observer<ObserverType, SubjectType>>::iterator i = observers.begin();
             i != observers.end();)
        {
    ...
    template <class ObserverType>
    template <class F, std::enable_if_t<!std::is_same_v<typename std::invoke_result_t<F, ObserverType&>, void>, int>>
    bool Subject<ObserverType, void>::NotifyObservers(F callback) const
    {
        for (typename IntrusiveList<Observer<ObserverType, SubjectType>>::const_iterator i = observers.begin();
             i != observers.end();)
        {
    ...
    template <class ObserverType>
    template <class F, std::enable_if_t<!std::is_same_v<typename std::invoke_result_t<F, ObserverType&>, void>, int>>
    bool Subject<ObserverType, void>::NotifyObservers(F callback)
    {
        for (typename IntrusiveList<Observer<ObserverType, SubjectType>>::iterator i = observers.begin();
             i != observers.end();)
        {
