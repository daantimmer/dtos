...
namespace infra
{
    ...
    {
        template <class T>
        struct IntrusiveSetNode: IntrusiveBinarySearchTreeNode<T>
        {
            IntrusiveSetNode();
    ...
    {
        static_cast<IntrusiveBinarySearchTree<T, Compare>&>(*this)
            = std::move(static_cast<IntrusiveBinarySearchTree<T, Compare>&>(other));
    ...
    bool IntrusiveSet<T, Compare>::invariant_holds() const
    {
        ...
        return (!this->RootNode() || invariant_holds_for_each_path(*this->RootNode(), true, blackLevels))
            && IntrusiveBinarySearchTree<T, Compare>::invariant_holds();
    }

    template <class T, class Compare>
    bool IntrusiveSet<T, Compare>::invariant_holds_for_each_path(const_reference node,
                                                                 bool mustBeBlack,
                                                                 std::size_t blackLevels) const
    {
        ...
        return ((!node.left && blackLevels == 0)
                || (node.left && invariant_holds_for_each_path(*node.left, mustBeBlack, blackLevels)))
            && ((!node.right && blackLevels == 0)
                || (node.right && invariant_holds_for_each_path(*node.right, mustBeBlack, blackLevels)));
    }
    ...
    void IntrusiveSet<T, Compare>::erase(const_reference value) // TICS !CFL#016
    {
...

        {
            ...
            if (oldReplacementParent != nullptr)
            {
                ...
                bool secondRed = oldReplacementParent == &value
                    ? twoChildren ? *oldReplacement && (*oldReplacement)->colour == Colour::red
                ...
                else
                {
                    ...
                    // a node being deleted with only one child is always black, since as red it cannot have a red child
                    // (two consecutive reds), nor a black child (one leaf and one black child is a violation of
                    // balance)
                }
            }
        ...
        {
            T* doubleBlackSibling
                = &doubleBlackParent->left == doubleBlack ? doubleBlackParent->right : doubleBlackParent->left;

            if (doubleBlackSibling->colour == Colour::black
                && ((doubleBlackSibling->left && doubleBlackSibling->left->colour == Colour::red)
                    || (doubleBlackSibling->right && doubleBlackSibling->right->colour == Colour::red)))
            {
                T** redNephew = (doubleBlackSibling->left && doubleBlackSibling->left->colour == Colour::red)
                    ? &doubleBlackSibling->left
                    : &doubleBlackSibling->right;
                ...
                doubleBlackParent = nullptr;
                     ...
                     && (!doubleBlackSibling->left || doubleBlackSibling->left->colour == Colour::black)
                     && (!doubleBlackSibling->right || doubleBlackSibling->right->colour == Colour::black))
            {
                // Step 3.2 b
