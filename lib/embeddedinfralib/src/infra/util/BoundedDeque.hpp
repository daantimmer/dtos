...
namespace infra
{
    ...
    public:
        ...
        BoundedDeque(infra::MemoryRange<infra::StaticStorage<T>> storage,
                     size_type n,
    ...
    infra::MemoryRange<T> BoundedDeque<T>::contiguous_range(const_iterator from)
    {
        if (!empty())
        {
            ...
            return infra::MemoryRange<T>(
                &*storage[i],
                std::min(&*storage[i] + numAllocated - from.index, &**storage.begin() + storage.size()));
        }
        else
            return infra::MemoryRange<T>();
    }
    ...
    template <class T>
    typename BoundedDeque<T>::iterator
        BoundedDeque<T>::insert(const const_iterator& position, size_type n, const value_type& value)
    {
    ...
    typename BoundedDeque<T>::iterator BoundedDeque<T>::insert(const const_iterator& position,
    ...
    namespace detail
    {
        ...
        template <class DequeType, class T>
        template <class DequeType2, class T2>
        BoundedDequeIterator<DequeType, T>&
            BoundedDequeIterator<DequeType, T>::operator=(const BoundedDequeIterator<DequeType2, T2>& other)
        {
            deque = other.deque;
            index = other.index;
