...
namespace infra
{
    ...
    {
        static_assert(sizeof(T) == sizeof(StaticStorage<T>),
                      "sizeof(StaticStorage) must be equal to sizeof(T) else reinterpret_cast will fail");

    private:
        struct Node: public detail::SharedPtrControl
        {
    ...
    public:
        ...
        using WithStorage
            = infra::WithStorage<SharedObjectAllocatorFixedSize,
                                 typename infra::BoundedVector<Node>::template WithMaxSize<NumberOfElements>>;
    ...
    template <class T, class... ConstructionArgs>
    typename SharedObjectAllocatorFixedSize<T, void(ConstructionArgs...)>::Node*
        SharedObjectAllocatorFixedSize<T, void(ConstructionArgs...)>::AllocateNode()
    {
        if (freeList)
        {
