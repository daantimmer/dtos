...
TEST(BoundedDequeTest, TestContiguousRange)
{
    ...
    EXPECT_EQ(0u, emptyDeque.contiguous_range(emptyDeque.begin()).size());
    EXPECT_EQ(0u, static_cast<const infra::BoundedDeque<int>&>(emptyDeque).contiguous_range(emptyDeque.begin()).size());

    int range[3] = {0, 1, 2};
    infra::BoundedDeque<int>::WithMaxSize<5> deque(range, range + 3);

    EXPECT_EQ(3u, deque.contiguous_range(deque.begin()).size());
    EXPECT_EQ(0, deque.contiguous_range(deque.begin()).front());

    deque.push_front(-1);
    deque.push_front(-2);

    EXPECT_EQ(2u, static_cast<const infra::BoundedDeque<int>&>(deque).contiguous_range(deque.begin()).size());
    EXPECT_EQ(-2, static_cast<const infra::BoundedDeque<int>&>(deque).contiguous_range(deque.begin()).front());
    EXPECT_EQ(1u, static_cast<const infra::BoundedDeque<int>&>(deque).contiguous_range(deque.begin() + 1).size());
    EXPECT_EQ(-1, static_cast<const infra::BoundedDeque<int>&>(deque).contiguous_range(deque.begin() + 1).front());
    EXPECT_EQ(3u, static_cast<const infra::BoundedDeque<int>&>(deque).contiguous_range(deque.begin() + 2).size());
