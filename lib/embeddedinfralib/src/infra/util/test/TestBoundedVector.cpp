...
TEST(BoundedVectorTest, TestRange)
{
    ...
    EXPECT_EQ(0u, emptyVector.range().size());
    EXPECT_EQ(0u, static_cast<const infra::BoundedVector<int>&>(emptyVector).range().size());

    int range[3] = {0, 1, 2};
    infra::BoundedVector<int>::WithMaxSize<5> vector(range, range + 3);

    EXPECT_EQ(3u, vector.range().size());
