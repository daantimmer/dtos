...
TEST(VariantTest, TestConstructionAtIndex)
{
    ...
    EXPECT_EQ(1u, v.Which());
    EXPECT_EQ(3u, v.Get<uint16_t>());
}
...
{
    struct Visitor: infra::StaticVisitor<void>
    {
        Visitor()
            : passed(0)
        {}
...
{
    struct Visitor: infra::StaticVisitor<bool>
    {
        bool operator()(bool)
        {
            return false;
...
struct DoubleVisitor: infra::StaticVisitor<int>
{
...
struct EmptyVisitor: infra::StaticVisitor<void>
{
    template <class T>
    void operator()(T)
    {}
