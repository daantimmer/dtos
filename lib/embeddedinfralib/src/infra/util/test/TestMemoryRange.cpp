...
TEST(MemoryRangeTest, IntersectingRange)
{
    ...
    EXPECT_EQ(infra::ByteRange(range.data() + 2, range.data() + 4),
              infra::IntersectingRange(infra::ByteRange(range.data(), range.data() + 4),
                                       infra::ByteRange(range.data() + 2, range.data() + 6)));
    EXPECT_EQ(infra::ByteRange(range.data() + 2, range.data() + 2),
              infra::IntersectingRange(infra::ByteRange(range.data(), range.data() + 2),
                                       infra::ByteRange(range.data() + 2, range.data() + 6)));
    EXPECT_EQ(infra::ByteRange(),
              infra::IntersectingRange(infra::ByteRange(range.data(), range.data() + 2),
                                       infra::ByteRange(range.data() + 3, range.data() + 6)));
    EXPECT_EQ(infra::ByteRange(),
              infra::IntersectingRange(infra::ByteRange(range.data() + 3, range.data() + 6),
                                       infra::ByteRange(range.data(), range.data() + 2)));
}
...
TEST(MemoryRangeTest, DiscardHead)
{
    ...
    EXPECT_EQ(infra::ByteRange(range.data() + 4, range.data() + range.size()),
              infra::DiscardHead(infra::ByteRange(range), 4));
...
TEST(MemoryRangeTest, Convert)
{
    ...
    EXPECT_EQ(0x04030201u, infra::Convert<uint32_t>(infra::MakeRange(range)));
}
...
TEST(MemoryRangeTest, TestCompare)
{
    ...
    EXPECT_FALSE(infra::ByteRange(range.data(), range.data() + 2)
                 == infra::ByteRange(range.data() + 1, range.data() + 2));
...
TEST(MemoryRangeTest, TestMakeRangeFromArray)
{
    ...
    EXPECT_EQ(infra::ByteRange(reinterpret_cast<uint8_t*>(&array), reinterpret_cast<uint8_t*>(&array) + 3),
              infra::MakeRange(array));
    EXPECT_EQ(infra::ConstByteRange(reinterpret_cast<const uint8_t*>(&constArray),
                                    reinterpret_cast<const uint8_t*>(&constArray) + 3),
              infra::MakeRange(constArray));
}

TEST(MemoryRangeTest, TestMakeRangeFromContainer)
{
    infra::BoundedVector<uint8_t>::WithMaxSize<3> container(
    ...
    EXPECT_EQ(infra::ByteRange(&container.front(), &container.front() + 3),
              infra::MakeRange(static_cast<const infra::BoundedVector<uint8_t>&>(container)));
}
...
TEST(MemoryRangeTest, MakeStringByteRange)
{
    ...
    EXPECT_EQ(infra::MemoryRange<const char>(string, string + std::strlen(string)),
              infra::ReinterpretCastMemoryRange<const char>(infra::MakeStringByteRange(string)));
    EXPECT_EQ(infra::MemoryRange<const char>(stdString.data(), stdString.data() + stdString.size()),
              infra::ReinterpretCastMemoryRange<const char>(infra::MakeStringByteRange(stdString)));
}
