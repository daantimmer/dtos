...
namespace
{
    struct SetInt: infra::IntrusiveSet<SetInt>::NodeType
    {
        SetInt(int v)
            : value(v)
        {}
...
TEST_F(IntrusiveSetTest, IterateAfterMoveAssignment)
{
    ...
    for (SetInt& i : copy)
    {
        EXPECT_THAT(i, ::testing::AnyOf(::testing::Eq(0), ::testing::Eq(1), ::testing::Eq(2)));
    }
    ...
    for (int size = 2; size != 40; ++size)
    {
        for (int step = 3; step != 7; ++step)
        {
            ...
            while (!indices.empty())
            {
                ...
                // std::cout << "insert size: " << size << ", step: " << step << ", set size: " << set.size() << ",
                // insert: " << indices[current] << std::endl;
            ...
            while (!set.empty())
            {
                ...
                // std::cout << "erase size: " << size << ", step: " << step << ", set size: " << set.size() << ",
                // erase: " << std::next(set.begin(), current)->value << std::endl;
