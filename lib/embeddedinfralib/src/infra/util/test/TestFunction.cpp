...
TEST(FunctionTest, ExecuteWithSize)
{
    ...
    EXPECT_CALL(m, callback(1, 2, 3));
    int a = 1, b = 2, c = 3;
    infra::Execute::WithExtraSize<4 * sizeof(void*)> execute([&m, a, b, c]() { m.callback(a, b, c); });
}
...
TEST(FunctionTest, ExecuteOnDestructionWithSize)
{
    ...
    {
        infra::ExecuteOnDestruction::WithExtraSize<4 * sizeof(void*)> execute([&m, a, b, c]() { m.callback(a, b, c); });
