...
namespace infra
{
    ...
    using StaticStorageForInheritanceTree
        = StaticStorageForPolymorphicObjects<Base,
                                             MaxSizeOfTypes<Derived...>::value - sizeof(Base),
                                             typename MaxAlignmentType<Derived...>::Type>;
    ...
    void StaticStorage<T>::Destruct() const
    {
        ...
        std::fill(const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(&data)),
                  const_cast<uint8_t*>(reinterpret_cast<const uint8_t*>(&data)) + sizeof(data),
    ...
    void StaticStorageForPolymorphicObjects<T, ExtraSize, AlignAs>::Construct(Args&&... args)
    {
        ...
        static_assert(std::alignment_of<U>::value <= sizeof(AlignAs),
                      "Alignment of U is larger than alignment of this function");
        static_assert(sizeof(U) <= sizeof(T) + ExtraSize,
