...
namespace infra
{
    ...
    public:
        ...
        bool operator==(std::nullptr_t) const;
        bool operator!=(std::nullptr_t) const;
        friend bool operator==(std::nullptr_t, const WeakPtr& ptr)
        {
            return ptr == nullptr;
        }
        friend bool operator!=(std::nullptr_t, const WeakPtr& ptr)
    ...
    class AccessedBySharedPtrWithExtraSize: private SharedObjectDeleter
    {
    public:
    ...
    {
        return SharedPtr<typename std::remove_const<T>::type>(
            sharedPtr.control,
            const_cast<typename std::remove_const<T>::type*>(sharedPtr.object));
    }

    template <class T>
    SharedPtr<typename std::remove_const<T>::type> ConstCast(SharedPtr<T>&& sharedPtr)
    {
        SharedPtr<typename std::remove_const<T>::type> result(
            sharedPtr.control,
            const_cast<typename std::remove_const<T>::type*>(sharedPtr.object));
        sharedPtr.Reset(nullptr, nullptr);
        return result;
    ...
    namespace detail
    {
        class NullAllocator: public SharedObjectDeleter
        {
        public:
            virtual void Destruct(const void* object) override;
            virtual void Deallocate(void* control) override;
    ...
    AccessedBySharedPtrWithExtraSize<ExtraSize>::AccessedBySharedPtrWithExtraSize(
        const infra::Function<void(), ExtraSize>& onUnReferenced)
    ...
    template <std::size_t ExtraSize>
    void AccessedBySharedPtrWithExtraSize<ExtraSize>::SetAction(
        const infra::Function<void(), ExtraSize>& newOnUnReferenced)
    {
    ...
    template <std::size_t ExtraSize>
    void AccessedBySharedPtrWithExtraSize<ExtraSize>::Destruct(const void*)
    {}

    template <std::size_t ExtraSize>
    void AccessedBySharedPtrWithExtraSize<ExtraSize>::Deallocate(void*)
    {
        onUnReferenced();
    }
    ...
    {
        template <class T>
        class SharedObjectOnHeap: private SharedObjectDeleter
        {
        public:
            template <class... Args>
            ...
            virtual ~SharedObjectOnHeap() = default;
